...
def login_required(handler_method):
    """A decorators to require that a user be logged in to access a handler.
    ...
    def check_login(self, *args, **kwargs):
        if self.request.method != 'GET':
            self.abort(400, detail='The login_required decorators '
...
def admin_required(handler_method):
    """A decorators to require that a user be an admin for this application
    to access a handler.
    ...
    def check_admin(self, *args, **kwargs):
        if self.request.method != 'GET':
            self.abort(400, detail='The admin_required decorators '
                'can only be used for GET requests.')
