...
class TaskQueueTasksHandler(admin_request_handler.AdminRequestHandler):
  ...
  @staticmethod
  def _get_tasks(queue_name, count, start_task_name=None, start_eta_usec=None):
    request = taskqueue_service_bytes_pb2.TaskQueueQueryTasksRequest()
    request.queue_name = queue_name
    request.max_rows = count
    if start_task_name is not None:
      request.start_task_name = start_task_name
    if start_eta_usec is not None:
      request.start_eta_usec = start_eta_usec

    response = taskqueue_service_bytes_pb2.TaskQueueQueryTasksResponse()
    apiproxy_stub_map.MakeSyncCall('taskqueue', 'QueryTasks', request, response)
  ...
  def _delete_task(queue_name, task_name):
    request = taskqueue_service_bytes_pb2.TaskQueueDeleteRequest()
    request.queue_name = queue_name
    request.task_name.append(task_name)

    response = taskqueue_service_bytes_pb2.TaskQueueDeleteResponse()
    apiproxy_stub_map.MakeSyncCall('taskqueue', 'Delete', request, response)
    return response.result(0)

  @staticmethod
  def _run_task(queue_name, task_name):
    request = taskqueue_service_bytes_pb2.TaskQueueForceRunRequest()
    request.queue_name = queue_name
    request.task_name = task_name

    response = taskqueue_service_bytes_pb2.TaskQueueForceRunResponse()
    apiproxy_stub_map.MakeSyncCall('taskqueue', 'ForceRun', request, response)
    return response.result

  def get(self, queue_name):
    super(TaskQueueTasksHandler, self).get()
    queue_info = next(
        taskqueue_utils.QueueInfo.get(queue_names=frozenset([queue_name])))
    ...
    self.response.write(
        self.render(
            'taskqueue_tasks.html', {
                'is_push_queue':
                    queue_info.mode ==
                    taskqueue_service_bytes_pb2.TaskQueueMode.PUSH,
                'page':
  ...
  def post(self, queue_name):
    ...
    if result == taskqueue_service_bytes_pb2.TaskQueueServiceError.UNKNOWN_QUEUE:
      message = 'Queue "%s" not found' % queue_name
    elif result == taskqueue_service_bytes_pb2.TaskQueueServiceError.UNKNOWN_TASK:
      message = 'Task "%s" not found' % task_name
    elif result != taskqueue_service_bytes_pb2.TaskQueueServiceError.OK:
      message = 'Internal error'
    else:
      message = ''
    self.redirect('%s?%s' % (self.request.path_url,
                             urllib.urlencode({
