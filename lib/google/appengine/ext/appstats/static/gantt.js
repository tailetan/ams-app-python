...
class Gantt {
  ...
  static compute_scale(highest) {
    ...
    for (var i = 0; i < Gantt.SCALES.length; i++) {
      if (highest <= Gantt.SCALES[i][2]) {
        return [
          Gantt.SCALES[i][0], Gantt.SCALES[i][1] * scale,
  ...
  draw_scale(howmany, spacing, scale) {
    this.write(
        '<tr class="' + this.PREFIX + 'axisrow">' +
        '<td width="20%"></td><td>');
    this.write('<div class="' + this.PREFIX + 'axis">');
    for (var i = 0; i <= howmany; i++) {
      this.write(
          '<img class="' + this.PREFIX + 'tick" src="' + this.PIX +
          '" alt="" ');
      this.write('style="left:' + (i * spacing * scale) + '%"\n>');
      this.write(
          '<span class="' + this.PREFIX +
          'scale" style="left:' + (i * spacing * scale) + '%">');
      this.write(
          '&nbsp;' + (i * spacing) + '</span>');  // TODO: number format %4g
    }
    ...
    for (var i = 0; i < this.bars.length; i++) {
      ...
      if (bar.label.length > 0) {
        if (bar.link_target.length > 0) {
          this.write(
              '<a class="' + this.PREFIX + 'link" href="' + bar.link_target +
              '">');
        }
      ...
      this.write(
          '<img class="' + this.PREFIX + 'bar" src="' + this.PIX + '" alt="" ');
      this.write(
          'style="left:' + (bar.start * scale) +
          '%;width:' + (bar.duration * scale) + '%;min-width:1px"\n>');
      if (bar.extra_duration > 0) {
        this.write(
            '<img class="' + this.PREFIX + 'extra" src="' + this.PIX +
            '" alt="" ');
        this.write(
            'style="left:' + (bar.start * scale) +
            '%;width:' + (bar.extra_duration * scale) + '%"\n>');
      }
      if (bar.inline_label.length > 0) {
        this.write(
            '<span class="' + this.PREFIX + 'inline" style="left:' +
            ((bar.start + Math.max(bar.duration, bar.extra_duration)) * scale) +
            '%">&nbsp;');
   ...
   *     suppress.
   * @param {string} inline_label Valid HTML or HTML-escaped text drawn after
   *     the bars; use '' to suppress.
   * @param {string} link_target HTML-escaped link where clicking on any element
   *       will take you; use '' for no linking.
   */
  add_bar(label, start, duration, extra_duration, inline_label, link_target) {
    this.highest_duration = Math.max(
        this.highest_duration,
        Math.max(start + duration, start + extra_duration));
    this.bars.push({
      label: label,
      start: start,
      duration: duration,
 ...
 */
Gantt.SCALES = [
  [5, 0.2, 1.0], [6, 0.2, 1.2], [5, 0.25, 1.25], [6, 0.25, 1.5], [4, 0.5, 2.0],
  [5, 0.5, 2.5], [6, 0.5, 3.0], [4, 1.0, 4.0], [5, 1.0, 5.0], [6, 1.0, 6.0],
  [4, 2.0, 8.0], [5, 2.0, 10.0]
];
