...
_GOOGLE3_FIELD_OPTIONS_EXTENSION = 337170254
  ...
  def _PrintFieldName(self, field):
    ...
    if self.use_field_number:
      ...
      if field.is_extension:
        ...
        if hasattr(field, 'GetOptions'):


          field_options = field.GetOptions()

          g3_options_extension = field_options.Extensions._FindExtensionByNumber(
              _GOOGLE3_FIELD_OPTIONS_EXTENSION)

          if (field_options.weak and g3_options_extension is not None and
              field_options.Extensions[g3_options_extension]
              .text_format_print_alt_name):
            out.write(field_options.Extensions[g3_options_extension].alt_name)
          else:
...
class _Parser(object):
  ...
  def _MergeField(self, tokenizer, message):
    ...
    if tokenizer.TryConsume('['):
      ...
      if not field:
        ...
        field = self._FindTextFormatAlias(message, name)
  ...
  def _FindTextFormatAlias(self, message, name):
    """Find the field in the message that has an alt_name annotation that matches the given name."""
    message_descriptor = message.DESCRIPTOR
    for field in message_descriptor.fields:
      field_options = field.GetOptions()
      if field_options.weak:

        g3_options_extension = field_options.Extensions._FindExtensionByNumber(
            _GOOGLE3_FIELD_OPTIONS_EXTENSION)

        if g3_options_extension is not None:
          if field_options.Extensions[
              g3_options_extension].alt_name == '[' + name + ']':
            return field
    return None
